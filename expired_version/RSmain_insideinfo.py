...
#sessionid
yourSessionid = "xxxxx"
#几号自习室，几号座位，几号自习室，几号座位
yourSeatInfo = "1,80,2,80"

#####################################################
#"1,80,2,80"表示你想要选1号自习室的80号座位和2号自习室的80号座位
#####################################################
...
def yanZhengConf():
	...
	try:
		...
		#使用内置配置信息
		dict_openID_SESSID = json.loads('{"user":"%s,%s"}'%(yourSessionid,yourSeatInfo))

	except Exception as e:
		print(u"[E]: 尝试读取配置文件出错 %s"%repr(e))
		return "#ERROR"
	...
	for v in dict_openID_SESSID.values():
		try:
			...
			print(u"[E]: 尝试验证sessionid出错 %s"%repr(e))


		#验证sessionid: 点击【座位】进入首页
		try:
			#进入首页
			respone=requests.get(url_zuowei,timeout=1,headers=mheaders,cookies=mcookies)
			...
			if (respone.status_code == 200 ) and ((respone.text.find("出入口")>0 or (respone.text.find("自习室")))):
				print(u"[I]: 尝试进入首页成功-状态码：%5d"% respone.status_code+"\n")
				#不出错则继续运行， 什么都不返回
				#return "#RIGHT"
			else:
				print(u"[I]: 尝试进入首页失败-状态码：%5d"% respone.status_code+"\n")
				return "#ERROR"
...
if __name__ == '__main__':
	...
	print(u"[I]: 图书馆抢座助手已启动...请尽量在开放抢座时间的前一小时内启动此脚本！\n")
	#先验证配置信息
	zhuangtai = yanZhengConf()
	if zhuangtai == "#ERROR" :
		print(u"[I]: 配置信息验证失败！请检查后重试...\n")
	else:
		print(u"[I]: 配置信息验证成功！程序继续执行...\n")
